...
class SimpleImage {
	...
	function resizeToSquare($size) {
		if ($this->getWidth() > $this->getHeight()) {
			$this->resizeToHeight($size);
			$this->crop(($this->getWidth()-$size) / 2, 0, $size, $size);
		} else {
			$this->resizeToWidth($size);
			$this->crop(0, ($this->getHeight()-$size) / 2, $size, $size);
		}
	}
	
	function perimeterScale($t) {
		$tc = $this->getWidth() + $this->getHeight();
		$rx = ($t/2) * ($this->getWidth() / $tc);
		$ry = ($t/2) * ($this->getHeight() / $tc);
		$this->resize($rx, $ry);
	}
	
	function fitInSquare($size) {
		if ($this->getWidth() > $this->getHeight()) {
			$this->resizeToWidth($size);
		} else {
			$this->resizeToHeight($size);
